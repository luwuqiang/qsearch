...
class CN_QuantifierSegmenter implements ISegmenter{
		...
		if(CharacterUtil.CHAR_CHINESE == context.getCurrentCharType()){
			...
			if(!this.countHits.isEmpty()){
				...
				for(Hit hit : tmpArray){
					...
					if(hit.isMatch()){
						//输出当前的词
						WordElement newWordElement = new WordElement(context.getBufferOffset() , hit.getBegin() , context.getCursor() - hit.getBegin() + 1 , WordType.TYPE_COUNT);
						context.addWordElement(newWordElement);
			...
			if(singleCharHit.isMatch()){//首字成量词词
				//输出当前的词
				WordElement newWordElement = new WordElement(context.getBufferOffset() , context.getCursor() , 1 , WordType.TYPE_COUNT);
				context.addWordElement(newWordElement);
	...
	private boolean needCountScan(AnalyzeContext context){
		if((nStart != -1 && nEnd != -1 ) || !countHits.isEmpty()){
			...
			//找到一个相邻的数词
			if(!context.getOrgWordElements().isEmpty()){
				WordElement we = context.getOrgWordElements().peekLast();
				if(WordType.TYPE_CNUM == we.getWordType() ||  WordType.TYPE_ARABIC == we.getWordType()){
					if(we.getBegin() + we.getLength() == context.getCursor()){
						return true;
					}
				}
			}
	...
	private void outputNumLexeme(AnalyzeContext context){
		if(nStart > -1 && nEnd > -1){
			//输出数词
			WordElement newLexeme = new WordElement(context.getBufferOffset() , nStart , nEnd - nStart + 1 , WordType.TYPE_CNUM);
			context.addWordElement(newLexeme);

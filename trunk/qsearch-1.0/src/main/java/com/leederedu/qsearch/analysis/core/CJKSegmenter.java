		...
		if(CharacterUtil.CHAR_USELESS != context.getCurrentCharType()){
			...
			if(!this.tmpHits.isEmpty()){
				...
				for(Hit hit : tmpArray){
					...
					if(hit.isMatch()){
						//输出当前的词
						WordElement newWordElement = new WordElement(context.getBufferOffset() , hit.getBegin() , context.getCursor() - hit.getBegin() + 1 , WordType.TYPE_CNWORD);
			...
			if(singleCharHit.isMatch()){//首字成词
				//输出当前的词
				WordElement newWordElement = new WordElement(context.getBufferOffset() , context.getCursor() , 1 , WordType.TYPE_CNWORD);
				context.addWordElement(newWordElement);

...
class AnalyzeContext {
    ...
    //原始分词结果集合，未经歧义处理
    private QuickSortSet orgWordElements;    
    //WordElementChain位置索引表
    private Map<Integer , WordElementChain> pathMap;    
    //最终分词结果集
    private LinkedList<WordElement> results;
...
//    	this.cfg = cfg;
    	...
    	this.orgWordElements = new QuickSortSet();
    	this.pathMap = new HashMap<Integer , WordElementChain>();    	
    	this.results = new LinkedList<WordElement>();
    }
...
//    }
	 ...
	 * @param we
	 */
	void addWordElement(WordElement we){
		this.orgWordElements.addWordElement(we);
	}
	 ...
	 * @param path
	 */
	void addWordElementChain(WordElementChain wec){
		if(wec != null){
			this.pathMap.put(wec.getPathBegin(), wec);
		}
	}
	 ...
	 * @return
	 */
	QuickSortSet getOrgWordElements(){
		return this.orgWordElements;
	}
	...
	void outputToResult(){
		...
		for( ; index <= this.cursor ;){
			...
			//从pathMap找出对应index位置的WordElementChain
			WordElementChain chain = this.pathMap.get(index);
			if(chain != null){
				//输出WordElementChain中的wordElement到results集合		
				WordElement we = chain.pollFirst();
				while(we != null){
					this.results.add(we);
					//将index移至wordElement后
					index = we.getBegin() + we.getLength();					
					we = chain.pollFirst();
					if(we != null){
						//输出path内部，词元间遗漏的单字
						for(;index < we.getBegin();index++){
							this.outputSingleCJK(index);
						}
					}
				}
			}else{//pathMap中找不到index对应的WordElementChain
				//单字输出
				this.outputSingleCJK(index);
				index++;
			}
	...
	private void outputSingleCJK(int index){
		if(CharacterUtil.CHAR_CHINESE == this.charTypes[index]){			
			WordElement singleCharWordElement = new WordElement(this.buffOffset , index , 1 , WordType.TYPE_CNCHAR);
			this.results.add(singleCharWordElement);
		}else if(CharacterUtil.CHAR_OTHER_CJK == this.charTypes[index]){
			WordElement singleCharWordElement = new WordElement(this.buffOffset , index , 1 , WordType.TYPE_OTHER_CJK);
			this.results.add(singleCharWordElement);
		}
	}
	 ...
	 * @return
	 */
	WordElement getNextWordElement(){
		//从结果集取出，并移除第一个Lexme
		WordElement result = this.results.pollFirst();
		while(result != null){
    		//数量词合并
    		...
    		if(Dictionary.getSingleton().isStopWord(this.segmentBuff ,  result.getBegin() , result.getLength())){
	    		...
	    		result.setWordText(String.valueOf(segmentBuff , result.getBegin() , result.getLength()));
	    		break;
    		}
		}
		return result;
	 ...
	 * 组合词元
	 */
	private void compound(WordElement result){
   		//数量词合并处理
		if(!this.results.isEmpty()){
			if(WordType.TYPE_ARABIC == result.getWordType()){
				WordElement nextLexeme = this.results.peekFirst();
				boolean appendOk = false;
				if(WordType.TYPE_CNUM == nextLexeme.getWordType()){
					//合并英文数词+中文数词
					appendOk = result.append(nextLexeme, WordType.TYPE_CNUM);
				}else if(WordType.TYPE_COUNT == nextLexeme.getWordType()){
					//合并英文数词+中文量词
					appendOk = result.append(nextLexeme, WordType.TYPE_CQUAN);
				}
				if(appendOk){
					//弹出
			...
			//可能存在第二轮合并
			if(WordType.TYPE_CNUM == result.getWordType() && !this.results.isEmpty()){
				WordElement nextWordElement = this.results.peekFirst();
				boolean appendOk = false;
				 if(WordType.TYPE_COUNT == nextWordElement.getWordType()){
					 //合并中文数词+中文量词
 					appendOk = result.append(nextWordElement, WordType.TYPE_CQUAN);
 				}  
				if(appendOk){
					//弹出
					this.results.pollFirst();   				

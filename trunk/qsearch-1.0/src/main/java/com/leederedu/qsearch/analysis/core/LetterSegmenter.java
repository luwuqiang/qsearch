...
class LetterSegmenter implements ISegmenter {
	...
	/* (non-Javadoc)
	 * @see cn.taneroom.lucene.analysis.core.ISegmenter#analyze(org.wltea.analyzer.core.AnalyzeContext)
	 */
	...
	/* (non-Javadoc)
	 * @see cn.taneroom.lucene.analysis.core.ISegmenter#reset()
	 */
	public void reset() {
		this.start = -1;
		this.end = -1;
	...
	private boolean processMixLetter(AnalyzeContext context){
		...
		if(this.start == -1){//当前的分词器尚未开始处理字符
			...
			if(CharacterUtil.CHAR_ARABIC == context.getCurrentCharType()
				...
				//遇到非Letter字符，输出词元
				WordElement newLexeme = new WordElement(context.getBufferOffset() , this.start , this.end - this.start + 1 , WordType.TYPE_LETTER);
	...
	private boolean processEnglishLetter(AnalyzeContext context){
		...
		if(this.englishStart == -1){//当前的分词器尚未开始处理英文字符	
			...
			if(CharacterUtil.CHAR_ENGLISH == context.getCurrentCharType()){
				...
				//遇到非English字符,输出词元
				WordElement newLexeme = new WordElement(context.getBufferOffset() , this.englishStart , this.englishEnd - this.englishStart + 1 , WordType.TYPE_ENGLISH);
				context.addWordElement(newLexeme);
				this.englishStart = -1;
				this.englishEnd= -1;
			}
	...
	private boolean processArabicLetter(AnalyzeContext context){
		...
		if(this.arabicStart == -1){//当前的分词器尚未开始处理数字字符	
			...
			if(CharacterUtil.CHAR_ARABIC == context.getCurrentCharType()){
				...
				////遇到非Arabic字符,输出词元
				WordElement newLexeme = new WordElement(context.getBufferOffset() , this.arabicStart , this.arabicEnd - this.arabicStart + 1 , WordType.TYPE_ARABIC);
				context.addWordElement(newLexeme);
				this.arabicStart = -1;
				this.arabicEnd = -1;
			}
